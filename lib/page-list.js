...
function updatePageList(batchOps) {
  return helpers.applyOpFilters(batchOps, setup.mappings, setup.settings, PAGES_INDEX, filterForPageOps)
    .then(function (resp) {
...
function updatePageListEntry(value) {
  return function (resp) {
    ...
    if (hit) {
      console.log('Trying to update page document at with data: ', value);
 ...
 * @return {Promise}
 */
function findPageAndUpdate(reqObj) {
  if (!reqObj.url) {
    throw new Error('Cannot find page without a url');
  }

  return elastic.query('pages', constructFindQuery(reqObj.url), 'general')
